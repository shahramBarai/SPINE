...
const apiBase =
    (import.meta.env.VITE_BUILDING_API_BASE_URL as string | undefined)?.replace(
        /\/$/,
        ""
...
const tryStartBackendViaVite = async (): Promise<boolean> => {
    ...
    try {
        const response = await fetch("/__dev/start-building-api", {
            method: "POST"
        });
        if (!response.ok) {
            return false;
...
const isUnknownNodeType = (value: string): boolean =>
...
const bindingValue = (row: SparqlBinding, key: string): string =>
    row[key]?.value ?? "";

const bindingType = (row: SparqlBinding, key: string): string =>
    row[key]?.type ?? "";

const parseTriplesFromBindings = (
    bindings: SparqlBinding[],
    maxCount: number
): { triples: Triple[]; graph: GraphData } => {
    ...
    for (const row of bindings) {
        ...
        const predicate =
            bindingValue(row, "p") || bindingValue(row, "predicate");
...
const fetchElementBatidLabels = async (
    uris: string[]
): Promise<Map<string, string>> => {
    const labels = new Map<string, string>();
    if (!uris.length) {
...
const applyFetchedTypesToGraph = (
    graph: GraphData,
    triples: Triple[],
    typesByUri: Map<string, string>
): GraphData => {
    if (!graph.nodes.length || !typesByUri.size) {
        const classLikeUris = new Set<string>();
        for (const triple of triples) {
            if (
                isRdfTypePredicate(triple.predicate) &&
                isLikelyUri(triple.object)
            ) {
        ...
        const nodes = graph.nodes.map((node) => {
            ...
            if (
                !uri ||
                !classLikeUris.has(uri) ||
                !isUnknownNodeType(node.type)
            ) {
                return node;
            }
...
const parseTriplesFromNTriples = (
    ntriples: string,
    maxCount: number
): { triples: Triple[]; graph: GraphData } => {
    const triples: Triple[] = [];
    ...
    for (const rawLine of ntriples.split(/\r?\n/)) {
        ...
        const subject = rawSubject.startsWith("<")
            ? rawSubject.slice(1, -1)
            : rawSubject;
        const predicate = rawPredicate.slice(1, -1);
        const object = rawObject.startsWith("<")
            ? rawObject.slice(1, -1)
            : rawObject;
        ...
        if (isRdfTypePredicate(predicate)) {
            const looksLikePrefixedType =
                /^[A-Za-z][A-Za-z0-9_-]*:[^\s]+$/.test(rawObject);
            if (
                rawObject.startsWith("<") ||
                rawObject.startsWith("_:") ||
                looksLikePrefixedType
            ) {
                rdfTypeByUri.set(subject, formatRdfTypeValue(object));
            }
        }
        ...
        const objectIsNode =
            rawObject.startsWith("<") || rawObject.startsWith("_:");
        if (!objectIsNode) {
            continue;
        }
...
export const loadIfc = async (
    request: Pick<PipelineRequest, "file" | "dir">
): Promise<IfcScanResult> =>
    postJson<IfcScanResult, Pick<PipelineRequest, "file" | "dir">>(
        "/pipeline/load-ifc",
        request
    );

export const convertIfcToTtl = async (
    request: PipelineRequest
): Promise<PipelineResult> =>
    postJson<PipelineResult, PipelineRequest>("/pipeline/convert", request);

export const syncIfcToFuseki = async (
    request: PipelineRequest
): Promise<PipelineResult> =>
...
export const runSemanticSearch = async (
    ...
    if (form === "other") {
        throw new Error(
            "Only SPARQL SELECT and CONSTRUCT queries are supported."
        );
    }
    ...
    try {
        ...
        throw new Error(
            "Cannot reach Fuseki query endpoint. Check that Fuseki is running."
        );
    }
    ...
    if (!response.ok) {
        ...
        throw new Error(
            `SPARQL query failed: ${response.status} ${response.statusText}. ${errorBody}`
        );
    }
    ...
    const graphWithTypes = applyFetchedTypesToGraph(
        parsed.graph,
        parsed.triples,
        typesByUri
    );
    ...
    return {
        ...
        graph: applyFetchedLabelsToGraph(
            graphWithTypes,
            parsed.triples,
            labelsByUri,
            elementBatidByUri
