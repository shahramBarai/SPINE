...
import {
    type GraphData,
    type EntityProperty,
    type SensorSelectionDetails,
    fetchEntityProperties,
    fetchSensorInformation,
    fetchSensorReadings,
    createSensorLiveStream
} from "@/lib/twin-api";
...
const isRdfsCommentPredicate = (predicate: string): boolean => {
    const lower = predicate.trim().toLowerCase();
    return (
        lower === RDFS_COMMENT_PREDICATE ||
        lower === "rdfs:comment" ||
        lower.endsWith("#comment") ||
        lower.endsWith("/comment")
    );
};
...
const typeDisplay = (value: string): string =>
...
const toCurieIfKnown = (value: string): string => {
    ...
    const ns = `${prefix}#`;
    if (ns === "https://w3id.org/bot#" || ns === "http://w3id.org/bot#")
        return `bot:${suffix}`;
    if (
        ns === "https://brickschema.org/schema/Brick#" ||
        ns === "https://brickschema.org/schema/brick#"
    )
        return `brick:${suffix}`;
    ...
    if (ns === "http://www.w3.org/1999/02/22-rdf-syntax-ns#")
        return `rdf:${suffix}`;
    if (ns === "https://w3id.org/fso#" || ns === "http://w3id.org/fso#")
        return `fso:${suffix}`;
...
export const GraphPane = ({
    ...
    const [sensorSelectionDetails, setSensorSelectionDetails] =
        useState<SensorSelectionDetails | null>(null);
    const [loadingSensorSelectionDetails, setLoadingSensorSelectionDetails] =
        useState(false);
    ...
    const [selectionPanelPosition, setSelectionPanelPosition] =
        useState<PanelPosition>({ x: 12, y: 208 });
            ...
            return graphData.nodes.map((rawNode) => {
                ...
                if (existing) {
                    return {
                        ...
                        label: rawNode.label,
                        type:
                            rdfTypeByNodeId.get(rawNode.id) ??
                            typeDisplay(rawNode.type)
                    };
                }
    ...
    const selectedNodeIsSensor = useMemo(() => {
        ...
        const typeLooksLikeSensor = (selectedNode?.type ?? "")
            .toLowerCase()
            .includes("sensor");
        const labelLooksLikeSensor = (selectedNode?.label ?? "")
            .toLowerCase()
            .includes("sensor");
    ...
    const selectedEdge = useMemo(() => {
        ...
        return (
            graphData.edges.find(
                (edge, index) =>
                    edgeKey(index, edge.from_id, edge.to_id, edge.label) ===
                    selectedEdgeKey
            ) ?? null
        );
    ...
    useEffect(() => {
        ...
        const uri = semanticTriples.reduce<string | null>((found, triple) => {
            ...
            if (
                triple.subject.endsWith(`#${selectedId}`) ||
                triple.subject.endsWith(`/${selectedId}`)
            ) {
                return triple.subject;
            }
            if (
                triple.object.endsWith(`#${selectedId}`) ||
                triple.object.endsWith(`/${selectedId}`)
            ) {
    ...
    useEffect(() => {
        ...
        fetchSensorInformation(selectedSensorTelemetryId ?? selectedId)
            .then((details) => {
                if (!cancelled) {
    ...
    useEffect(() => {
        ...
        const source = createSensorLiveStream(
            "8d9efb3a-887e-407b-997b-c92326c46d95"
        );
        ...
        source.addEventListener("message", (event) => {
            ...
            if (message.measurement.value !== undefined) {
                setLiveLatestValue(message.measurement.value ?? null);
            }
            if (message.measurement.valid_to !== undefined) {
                setLiveLatestTime(message.measurement.valid_to ?? null);
            }
        });
    ...
    useEffect(() => {
                    ...
                    .map((row) => {
                        const rawValue =
                            row.data && typeof row.data === "object"
                                ? (row.data as Record<string, unknown>).value
                                : undefined;
                        const value =
        ...
        selectedSensorTelemetryId
    ]);

    const displayedLatestValue =
        liveLatestValue ?? sensorSelectionDetails?.latest_value ?? null;
    const displayedLatestTime =
        liveLatestTime ?? sensorSelectionDetails?.latest_time ?? null;
    ...
    const searchMatches = useMemo(() => {
        ...
        graphData.edges.forEach((edge, index) => {
            if (
                ...
                if (!directNodeIds.has(edge.from_id))
                    edgeEndpointNodeIds.add(edge.from_id);
                if (!directNodeIds.has(edge.to_id))
                    edgeEndpointNodeIds.add(edge.to_id);
            }
        });
        ...
        graphData.edges.forEach((edge) => {
            if (
                directNodeIds.has(edge.from_id) &&
                !directNodeIds.has(edge.to_id)
            )
                neighborNodeIds.add(edge.to_id);
            if (
                directNodeIds.has(edge.to_id) &&
                !directNodeIds.has(edge.from_id)
            )
                neighborNodeIds.add(edge.from_id);
        });
    ...
    const fitToScreen = () => {
        ...
        const source = searchMatches
            ? nodes.filter((n) => searchMatches.directNodeIds.has(n.id))
            : nodes;
        if (!source.length) {
            return;
        }
        ...
        const fitZoom = Math.max(
            ...
            Math.min(
                3.5,
                Math.min((700 - margin) / width, (420 - margin) / height)
            )
        );
        ...
        return [
            {
                label: "From",
                value:
                    nodeMap.get(selectedEdge.from_id)?.label ??
            ...
            { label: "Predicate", value: selectedEdge.label },
            {
                label: "To",
                value:
                    nodeMap.get(selectedEdge.to_id)?.label ?? selectedEdge.to_id
            }
        ];
        ...
        >
                    ...
                    >
                        <div>
                            ...
                            <div className="text-[10px] text-muted-foreground/80">
                                {selectedEdge
                                    ? "Drag this panel away from the selected link."
                                    : "Drag this panel away from the selected item."}
                            </div>
                        </div>
                    ...
                    <div className="max-h-40 overflow-auto px-3 py-2">
                        {selectedEdge ? (
                            ...
                            <div className="space-y-1 text-[11px]">
                                ...
                                {!loadingSensorSelectionDetails &&
                                    selectedNodeIsSensor &&
                                    sensorSelectionDetails?.telemetry_error && (
                                        <div className="pt-1 text-[10px] font-mono text-muted-foreground">
                                            Telemetry lookup issue:{" "}
                                            {
                                                sensorSelectionDetails.telemetry_error
                                            }
                                        </div>
                                ...
                                {filteredProperties.map((prop, idx) => {
                                    const propName = prop.property.includes("#")
                                        ? prop.property.split("#").pop()
                                        : prop.property.split("/").pop();
                                    const valueDisplay =
                                        prop.value.length > 60
                                            ? `${prop.value.substring(0, 57)}...`
                                            : prop.value;
                                ...
                                {!loadingProperties &&
                                    filteredProperties.length === 0 && (
                                        <div className="pt-1 text-[10px] font-mono text-muted-foreground">
                                            No additional RDF properties found
                                            for this node.
                                        </div>
                                    )}
                            </div>
            ...
            <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
                <DialogContent className="max-w-3xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Full history for sensor{" "}
                            {selectedSensorTelemetryId ?? selectedId ?? "-"}
                        </DialogDescription>
                    </DialogHeader>
            ...
            <div className="absolute inset-0 grid-bg">
                ...
                >
                    ...
                    <g
                        transform={`translate(${pan.x} ${pan.y}) translate(350 210) rotate(${rotationDeg}) scale(${zoom}) translate(-350 -210)`}
                    >
                        {/* Edges */}
                        {graphData.edges.map((e, i) => {
                            ...
                            const edgeMatchesSearch =
                                searchMatches?.edgeKeys.has(key) ?? false;
                            const edgeNeighborsMatchedNode =
                                (searchMatches?.directNodeIds.has(e.from_id) ||
                                    searchMatches?.directNodeIds.has(
                                        e.to_id
                                    )) ??
                                false;
                            ...
                            const showLabel =
                                selected ||
                                edgeMatchesSearch ||
                                edgeNeighborsMatchedNode ||
                                (!!selectedId && relatedToSelectedNode);
                            return (
                                <g
                                    key={key}
                        ...
                        {nodes.map((n) => {
                            ...
                            const relatedToSelectedNode =
                                isNodeRelatedToSelectedNode(n.id);
                            const relatedToSelectedEdge =
                                isNodeRelatedToSelectedEdge(n.id);
                            const { fill, stroke: baseStroke } = colorForType(
                                n.type
                            );
                            ...
                            if (searchMatches) {
                                // Search is active: mirror selection highlight rules.
                                const isDirectMatch =
                                    searchMatches.directNodeIds.has(n.id);
                                const isNeighbor =
                                    searchMatches.neighborNodeIds.has(n.id);
                                const isEdgeEndpoint =
                                    searchMatches.edgeEndpointNodeIds.has(n.id);
                    ...
                    .map((t) => (
                        <span key={t} className="flex items-center gap-1.5">
                            <span
                                ...
                                style={{ background: colorForType(t).stroke }}
                            />
                            {splitCamelCase(t)}
                        </span>
                    ))}
            ...
            <div className="absolute bottom-3 right-3 glass rounded-md px-2.5 py-1 font-mono text-[10px] text-muted-foreground">
                {nodes.length} nodes · {graphData.edges.length} edges · zoom{" "}
                {zoom.toFixed(2)}x · rot {rotationDeg.toFixed(0)}° ·{" "}
                {physicsEnabled ? "live" : "frozen"}
            </div>
        </div>
    );
};
