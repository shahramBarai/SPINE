...
/** The draggable node palette; drops are handled by FlowView's `onDrop`. */
const Sidebar = memo(() => {
    const onDragStart = (
            ...
            <div className="space-y-2">
                {nodeItems.map((node) => (
                    ...
                    >
                        <div
                            className={cn(
                                "text-xs border-2 rounded-md flex items-center mx-1",
                                "border-border hover:border-primary group"
                            )}
                        >
                            {node.type !== NodeType.SOURCE && (
                                <div className="z-10 w-2 h-5 rounded-none border-2 bg-primary/70 border-surface group-hover:bg-primary ml-[-6px]" />
                            )}
                            ...
                            {node.type !== NodeType.SINK && (
                                <div className="z-10 w-2 h-5 rounded-none border-2 bg-primary/70 border-surface group-hover:bg-primary mr-[-6px]" />
                            )}
                        </div>
                    </div>
                ))}
