            ...
            <TitledCard title="Connection Status" isLoading={false}>
                <div className="flex items-center gap-2 justify-between">
                    ...
                    <div className="relative">
                        <KafkaLogo className="w-10 h-10 text-foreground" />
    ...
    return (
        <TitledCard title="Connection Status" isLoading={isLoading}>
            <div className="flex items-center gap-2 justify-between">
                <div className="flex flex-col">
                    <span
                        className={`text-lg font-semibold ${isConnected ? "text-foreground" : "text-danger"}`}
                    >
                        {isConnected ? "Connected" : "Disconnected"}
                    </span>
                    {isConnected && data?.brokersCount ? (
                        <p className="text-xs text-muted-foreground mt-0.5">
                            {data.brokersCount} broker
                            {data.brokersCount !== 1 ? "s" : ""} active
                        </p>
                    ) : (
                        <p className="text-xs text-muted-foreground mt-0.5">
