...
import { type ViewerComponentInfo } from "./ViewerPane";
...
export const SelectedElementPanel = ({
  ...
  liveMode: boolean;
  viewerComponentInfo: ViewerComponentInfo | null;
}) => {
          ...
          <>
            <div className="mb-2">
              <div className="text-[9px] uppercase font-mono text-primary tracking-wider mb-0.5">
                {viewerComponentInfo.type || "IFC Component"}
              </div>
              <div className="text-xs font-semibold leading-tight">{viewerComponentInfo.name || node?.name || "Unnamed component"}</div>
              <div className="text-[9px] font-mono text-muted-foreground mt-0.5 truncate">
                {viewerComponentInfo.modelName}
              </div>
            </div>

            <div className="grid grid-cols-2 gap-1.5 mb-2">
              <div className="bg-secondary/50 rounded p-1.5">
                <div className="text-[9px] uppercase text-muted-foreground font-mono">Express ID</div>
                <div className="font-mono text-[10px] truncate">{viewerComponentInfo.expressId}</div>
              </div>
              <div className="bg-secondary/50 rounded p-1.5">
            ...
            <div className="mb-2 rounded bg-secondary/50 p-1.5">
              <div className="text-[9px] uppercase text-muted-foreground font-mono">GlobalId</div>
              <div className="font-mono text-[10px] break-all">{viewerComponentInfo.globalId || "-"}</div>
            </div>

            <div className="mb-2 rounded border border-border/50 overflow-hidden">
              <div className="border-b border-border/50 bg-secondary/40 px-1.5 py-1 text-[9px] font-mono uppercase text-muted-foreground">
                IFC Properties
              </div>
              <div className="max-h-40 overflow-y-auto">
                {viewerComponentInfo.loading ? (
                  <div className="px-1.5 py-2 text-[10px] text-muted-foreground">Loading IFC properties...</div>
                ) : viewerComponentInfo.error ? (
                  <div className="px-1.5 py-2 text-[10px] text-destructive">{viewerComponentInfo.error}</div>
                ) : viewerComponentInfo.rows.length === 0 ? (
                  <div className="px-1.5 py-2 text-[10px] text-muted-foreground">No property set data found for this component.</div>
                ) : (
                  viewerComponentInfo.rows.map((row, index) => (
                    <div key={`${row.group}:${row.name}:${index}`} className="border-t border-border/40 px-1.5 py-1 first:border-t-0">
                      <div className="text-[9px] font-mono text-muted-foreground truncate">{row.group}</div>
                      <div className="text-[10px]">{row.name}</div>
                      <div className="text-[10px] text-muted-foreground break-all">{row.value}</div>
                    </div>
                  ))
                )}
              </div>
