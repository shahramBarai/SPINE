...
async function addMembers(
    entityId: string,
    newMembers: { userId: string; role: MemberRole }[]
) {
    // Check if users exist
    const allUsersExist = await getAllUsers();
    const unfoundUsers = newMembers.filter(
        (member) => !allUsersExist.some((user) => user.id === member.userId)
    );
    if (unfoundUsers.length > 0) {
        throw new Error(
            `Users with IDs ${unfoundUsers.map((user) => user.userId).join(", ")} not found`
        );
    }
...
async function updateMemberRole(
    ...
    if (!member) {
        throw new Error(
            `User with ID ${userId} is not a member of entity ${entityId}`
        );
    }
